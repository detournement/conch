...
4. Explicit composers — ``read_paste_block`` (/paste: raw tty lines until a
   lone "." or Ctrl+D) and ``edit_in_editor`` (/edit: $EDITOR on a temp
   file) work on any terminal and any backend.
...
def resolve_editor(config: Optional[dict]) -> str:
    """Editor precedence: config `editor`, then $VISUAL, then $EDITOR, then vi."""
    candidates = (
