...
CHAT_SYSTEM_PROMPT = (
    ...
    "- local_shell: you have a built-in tool that runs commands on the user's machine. "
    "  Use it for ANY local task: nmap, curl, docker, git, kubectl, file operations, "
    "  writing scripts, setting up cron jobs, running background processes, etc.\n"
    "  KEY BEHAVIORS:\n"
    "  - You CAN and SHOULD execute multi-step tasks autonomously. Don't just suggest "
    "    commands — run them yourself via local_shell.\n"
    "  - You can write files (scripts, configs), make them executable, set up cron jobs, "
    "    create launchd plists, run background processes with nohup/&, use 'watch', etc.\n"
    "  - For scheduled/recurring tasks: write a script, then install a cron job. Do it "
    "    yourself — don't ask the user to do it manually.\n"
    "  - Chain freely: run a command, use the output with MCP tools (e.g. scan a host "
...
LOCAL_SHELL_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            "stdout + stderr. Use for ANY local task: running commands, writing "
            "scripts/files, setting up cron jobs, background processes, etc. "
            "You can chain multiple calls: write a script, chmod +x it, install "
            "a cron job, run background tasks with nohup. Do things autonomously — "
            "don't suggest commands, execute them yourself."
        ),
        "parameters": {
            "type": "object",
            "properties": {
