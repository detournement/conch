...
CONCH_CONFIG_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            "get_registry/set_registry/clear_registry manage the llama-idx "
            "fleet registry (llamaidx_url): set_registry validates the URL "
            "before persisting it, and its models then appear in list_models "
            "as llamaidx/<provider>/<model>."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "value": {
                    "type": "string",
                    "description": (
                        "The value to set (model name, provider name, "
                        "registry URL, on/off, number)"
                    ),
                },
            },
            "required": ["action"],
...
class ConchConfigClient:
    ...
    def call_tool(self, name: str, arguments: dict) -> dict:
        ...
        if action == "get":
            ...
            from .llamaidx import get_llamaidx_url as _registry_url

            if _registry_url(self._config):
                lines.append(f"llamaidx_url: {_registry_url(self._config)}")
        ...
        # Registry management mirrors /registry exactly: same validation
        # (probe before commit), same persistence (set_config_values into
        # the primary config file), applied immediately — wiring a registry
        # is safe mid-turn, unlike a model switch.
        if action == "get_registry":
            from .llamaidx import get_llamaidx_url, probe_llamaidx_registry, registry_probe_summary

            url = get_llamaidx_url(self._config)
            if not url:
                return self._text(
                    "No llama-idx registry configured (llamaidx_url is "
                    "unset). Use action=set_registry with the registry URL "
                    "to wire one."
                )
            ok, reason, status = probe_llamaidx_registry(url, self._config)
            if not ok:
                return self._text(f"llamaidx_url: {url}\nstatus: {reason}")
            return self._text(
                f"llamaidx_url: {url}\n"
                f"status: reachable — {registry_probe_summary(status)}\n"
                "Selectable entries appear in action=list_models as "
                "llamaidx/<provider>/<model>; action=set_model with such a "
                "name switches to one."
            )

        if action == "set_registry":
            from .llamaidx import (
            ...
            if not value:
                return self._text("Error: provide the registry URL in 'value'")
            url = get_llamaidx_url({"llamaidx_url": value})
            ok, reason, status = probe_llamaidx_registry(url, self._config)
            if not ok:
                return self._text(
                    f"Not saved: {reason}. Checked {url}/v1/inference — "
                    "llamaidx_url is unchanged."
                )
            from .config import set_config_values

            path = set_config_values({"llamaidx_url": url})
            self._config["llamaidx_url"] = url
            clear_llamaidx_cache()
            return self._text(
                f"llamaidx_url set to {url} (persisted to {path}).\n"
                f"{registry_probe_summary(status)}\n"
                "Its models now appear in action=list_models as "
                "llamaidx/<provider>/<model>; action=set_model with such a "
                "name switches to one."
            )

        if action == "clear_registry":
            from .llamaidx import clear_llamaidx_cache, get_llamaidx_url

            current = get_llamaidx_url(self._config)
            if not current:
                return self._text(
                    "No llama-idx registry configured; nothing to clear."
                )
            from .config import set_config_values

            path = set_config_values({"llamaidx_url": ""})
            self._config["llamaidx_url"] = ""
            clear_llamaidx_cache()
            return self._text(
                f"Registry removed (was {current}; llamaidx_url cleared in "
                f"{path}). llamaidx/... models are no longer selectable."
            )
