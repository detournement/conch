...
The MV3 extension in ``satellites/browser-capture/`` observes DOM
...
def host_command_path() -> str:
    """Absolute path to the installed ``conch-capture-host`` script."""
    sibling = Path(sys.executable).parent / "conch-capture-host"
    if sibling.exists():
        return str(sibling)
    found = shutil.which("conch-capture-host")
    if found:
        return str(Path(found).resolve())
    raise BrowserEventRejected(
        "conch-capture-host is not installed on PATH — reinstall conch "
        "(pip install -e .) so the console script exists"
    )


def host_manifest(extension_id: str = "") -> Dict[str, Any]:
...
def install_native_host(extension_id: str = "") -> Dict[str, Any]:
    ...
    """
    manifest = host_manifest(extension_id)
...
def extension_dir() -> Optional[Path]:
    """The satellite extension directory when running from a checkout
    (the wheel deliberately excludes ``satellites/``)."""
    candidate = (
        Path(__file__).resolve().parent.parent.parent
        / "satellites" / "browser-capture" / "extension"
    )
