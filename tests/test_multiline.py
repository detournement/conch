...
class EditInEditorTests(unittest.TestCase):
    ...
    def test_editor_precedence_shared_resolver(self):
        # One resolver for every editor-backed surface: /edit rides
        # conch.config.resolve_editor (the /notes chain, nano floor
        # included) and multiline keeps no duplicate of its own.
        from conch import config as config_mod

        self.assertFalse(hasattr(multiline, "resolve_editor"))
        env = {"VISUAL": "visual-editor", "EDITOR": "editor-editor"}
        with patch.dict(os.environ, env, clear=False):
            self.assertEqual(config_mod.resolve_editor({"editor": "cfg"}), "cfg")
            self.assertEqual(config_mod.resolve_editor({}), "visual-editor")
        with patch.dict(os.environ, {"EDITOR": "editor-editor"}, clear=False):
            os.environ.pop("VISUAL", None)
            self.assertEqual(config_mod.resolve_editor(None), "editor-editor")
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VISUAL", None)
            os.environ.pop("EDITOR", None)
            with patch("conch.config.shutil.which", return_value="/usr/bin/nano"):
                self.assertEqual(config_mod.resolve_editor(None), "nano")
            with patch("conch.config.shutil.which", return_value=None):
                self.assertEqual(config_mod.resolve_editor(None), "vi")
