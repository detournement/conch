...
# ---------------------------------------------------------------------------
# Fleet commands (the fleet awakening): /fleet drives the distributed task
# plane — against the conch-controller daemon when it runs, direct-drive
# (one-shot schedule + poll) when it doesn't; the same two-transport client
# abstraction the mission commands use. conch.fleet imports stay lazy and
# gated on fleet_controller=true so shell-only users never load it.
# ---------------------------------------------------------------------------
...
def _fleet_attach(config: dict):
    ...
    if not get_bool(config, "fleet_controller"):
        ...
        return None
    try:
        from .fleet.client import attach_fleet
...
def _fleet_run(client, tokens: List[str], config: dict) -> None:
    from .fleet.client import run_fleet_task
...
def _handle_fleet_command(arg: str, config: dict) -> None:
    arg = (arg or "").strip()
...
def _fleet_enroll(client, rest: List[str], config: dict) -> None:
    ...
    if len(rest) < 2:
        print("\n  \033[2mUsage: /fleet enroll <name> <user@host>"
              " [port]\033[0m\n")
        return
    from .fleet.client import DirectFleetClient
    from .fleet.enroll import Enroller, EnrollmentError, build_ssh_target
...
def handle_slash_command(
    ...
    if command == "/fleet":
        _handle_fleet_command(arg, config)
        return None
